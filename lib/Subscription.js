...
Subscription.prototype.load = function (callback) {
  ...
  this.muzzley.on('connect', function (err) {
    ...
    profileSubscription.on('message', function (message, messageCallback) {
      ...
      if (payload.io === 'r') {
        self.emit('readRequest', {channelId: payload.channel, componentId: payload.component, property: payload.property, user: user}, messageCallback);
      }

      if (payload.io === 'w') {
        self.emit('readRequest', {channelId: payload.channel, componentId: payload.component, property: payload.property, value: value, user: user});
      }
