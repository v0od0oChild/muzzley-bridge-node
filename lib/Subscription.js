...
Subscription.prototype.load = function (callback) {
  ...
  this.muzzley.on('connect', function (err) {
    ...
    profileSubscription.on('message', function (message, messageCallback) {
      ...
      if (payload.io === 'r') {
        self.emit('read', user, {channelId: payload.channel, componentId: payload.component, property: payload.property}, messageCallback);
      }

      if (payload.io === 'w') {
        self.emit('write', user, {channelId: payload.channel, componentId: payload.component, property: payload.property, value: payload.data.value});
      }
    });
    connected = true;
    callback();
